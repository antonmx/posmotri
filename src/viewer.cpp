  ...
  QGLViewer(parent)
{
  ...
  connect(&m_autoUpdateTimer, SIGNAL(timeout()), SLOT(updateGL()));

  camera()->setIODistance(0.06f);
  camera()->setPhysicalScreenWidth(4.3f);

  connect(this, SIGNAL(pointSelected(const QMouseEvent*)), SLOT(checkPointSelected(const QMouseEvent*)));
  connect(this, SIGNAL(processMops()), SLOT(processMorphologicalOperations()));
  ...
  connect(this, SIGNAL(showMessage(QString, bool)), m_messageDisplayer, SLOT(holdMessage(QString, bool)));
  connect(m_messageDisplayer, SIGNAL(updateGL()), SLOT(updateGL()));
